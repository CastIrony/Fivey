...
struct GraphMinorGridLines: Shape
{
    let poll: Poll?
    ...
    func path(in rect: CGRect) -> Path {
        ...
        for percent: CGFloat in stride(from: 0.1, through: 0.9, by: 0.1) {
            let y = rect.minY + rect.height * percent
...
struct WinProbabilityGraph: View
{
    ...
    var body: some View {
        ZStack {
            GraphMinorGridLines(poll: poll).stroke(Color(UIColor.secondarySystemGroupedBackground).opacity(0.4), lineWidth: 1)
